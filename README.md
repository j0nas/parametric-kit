...
| `parametric-kit/params`  | Schema-driven params: `defineParams` + `num`/`pick`/`toggle` → inferred `Params` type, `defaults`, `sanitize`, versioned localStorage store, `renderPanel` (groups, sliders, selects, dynamic `maxKey` ceilings, conditional visibility, hints), presets with "Custom…" fallback |
...
| `parametric-kit/testkit` | Geometry test probes for consuming apps' suites: `bbox`, `volume` (signed-tetra, mm³), `verticesOnDisc`                                                                                                                                                                          |
