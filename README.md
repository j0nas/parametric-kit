...
| `parametric-kit/params`  | Schema-driven params: `defineParams` + `num`/`pick`/`toggle` → inferred `Params` type, `defaults`, `sanitize`, versioned localStorage store, `renderPanel` (groups, sliders, selects, dynamic `maxKey` ceilings, conditional visibility, hints, collapsible `<details>` accordion with persisted open state), presets with "Custom…" fallback |
